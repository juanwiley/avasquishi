...
export default function Header() {
  // If the provider isn’t ready yet, default to 0 so the header stays stable.
  const { totalQty = 0, lastAddedAt } = useCart?.() ?? {};
  const [ping, setPing] = useState(false);

  // Re-trigger small pop each time an item is added/removed.
  useEffect(() => {
      ...
      <div className={s.inner}>
        ...
        <div className={s.actions} role="navigation" aria-label="Account and cart">
          ...
          <Link href="/cart" className={s.iconButton} aria-label="Cart">
            ...
            {totalQty > 0 && (
              <span key={String(lastAddedAt)} className={`${s.badge} ${ping ? s.badgePing : ""}`}>
                {totalQty}
              </span>
            )}
          </Link>
...
function CartIcon({ className }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path d="M3 3h2l2.4 12.1a2 2 0 0 0 2 1.6h7.3a2 2 0 0 0 2-1.6L21 7H6"
